...
if (pickUp) {
	if (y < 3*obj_camera.view_height/4 * dest_treshold) {
		image_xscale = start_xscale * cos(current_time / 400);
		x = lerp(x, obj_camera.view_width/2, 0.03);
		y = lerp(y, 3*obj_camera.view_height/4, 0.01);
	} else if (y < y_dest * dest_treshold || x < (100 + x_step * item_id) * dest_treshold) {
		image_alpha = lerp(image_alpha, 0.3, 0.05);
		image_xscale = lerp(image_xscale, start_xscale, 0.2);
		x = lerp(x, x_dest, 0.03);
		y = lerp(y, y_dest, 0.01);
	} else {
		x = x_dest;
		y = y_dest;
		...
		draw_text(x, y, string(obj_game.getItemAmount(obj_game.inventory_temp, item_id)));
		draw_set_alpha(1)
